...
async function createPharmacienAction(request, response) {
  try {
    const { email, password, firstname, lastname, rpps } = request.body;
    ...
    if (user_id != null) {
      ...
      const pharmacien = { firstname, lastname, rpps };
      ...
      if (pharmacien_id != null) {
        ...
        const result = await accountTypeRepo.addAccountType(user_id, pharmacien_id, "pharmacien");
