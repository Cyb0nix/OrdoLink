...
async function createMedecinAction(request, response) {
  try {
    if (!(await userRepo.checkExistsUser(request.body.email))) {
      ...
      const user_id = await userRepo.createUser(
        request.body.email,
        hashed_password,
        true,
      ...
      if (user_id != null) {
        ...
        console.log(medecin_id);
        if (medecin_id != null) {
          const result = await account_typeRepo.createAccountType(
            user_id,
            medecin_id,
            "medecin"
          );
          if (result != null) {
            ...
            response
              .status(200)
              .json({
                info: "medecin created successfully",
...
async function getMedecinByIdAction(request, response) {
  try {
    ...
    if (result != null) {
      response
        .status(200)
        .json({ info: "medecin found successfully", medecin: result });
    } else {
      response.status(400).json({ error: "invalid request" });
    }
  } catch (err) {
...
async function getMedecinsAction(request, response) {
  try {
    ...
    if (result != null) {
      response
        .status(200)
        .json({ info: "medecins found successfully", medecins: result });
    } else {
      response.status(400).json({ error: "invalid request" });
    }
  } catch (err) {
...
async function updateMedecinAction(request, response) {
  try {
    const result = await medecinRepo.updateMedecin(
      request.params.id,
      request.body.rpps,
      ...
      request.body.rpps_expiration_date,
      request.body.type_id
    );
    if (result != null) {
      response
        .status(200)
        .json({ info: "medecin updated successfully", medecin: result });
    } else {
      response.status(400).json({ error: "invalid request" });
    }
  } catch (err) {
...
async function deleteMedecinAction(request, response) {
  try {
    ...
    if (result != null) {
      response
        .status(200)
        .json({ info: "medecin deleted successfully", medecin: result });
    } else {
      response.status(400).json({ error: "invalid request" });
    }
  } catch (err) {
  ...
  try {
    ...
    if (result != null) {
      response
        .status(200)
        .json({
          info: "medecin patients found successfully",
          patients: result,
        });
    } else {
      response.status(400).json({ error: "invalid request" });
  ...
  try {
    ...
    if (result != null) {
      response
        .status(200)
        .json({
          info: "medecin ordonnaces found successfully",
          ordonnaces: result,
        });
    } else {
      response.status(400).json({ error: "invalid request" });
...
async function getTypesAction(request, response) {
  try {
    ...
    if (result != null) {
      response
        .status(200)
        .json({ info: "types found successfully", types: result });
    } else {
      response.status(400).json({ error: "invalid request" });
    }
  } catch (err) {
